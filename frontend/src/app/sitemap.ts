...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const entryGroups = await Promise.all([
    ...
    ...categories.map((category) =>
      bilingualEntry(`/products/category/${category.slug}`, { changeFrequency: 'weekly', priority: 0.6 }),
    ),
    ...products.items.map((product) => bilingualEntry(`/products/${product.slug}`, { changeFrequency: 'weekly', priority: 0.6 })),
    ...posts.items.map((post) => bilingualEntry(`/blog/${post.slug}`, { changeFrequency: 'monthly', priority: 0.5 })),
  ]);
