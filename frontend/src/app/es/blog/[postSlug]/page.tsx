...
export default async function SpanishBlogDetailPage({
  params,
}: {
  params: Promise<{ postSlug: string }>;
}) {
  ...
  return (
    <Container className="py-12">
      ...
      <article className="mt-6 grid gap-10 lg:grid-cols-[1fr_260px]">
        <div>
          {post.category && (
            <Link href={`/es/blog/category/${post.category.slug}`} className="text-sm font-medium text-water-600">
              {post.category.name}
            </Link>
          )}
          <h1 className="mt-2 text-3xl font-semibold text-navy-950">{post.title}</h1>
          <p className="mt-2 text-sm text-grey-500">
            {t('es', 'byAuthorPrefix')} {post.authorName} · {formatDate(post.publishedAt)}
          </p>
      ...
      <RelatedPosts posts={related} locale="es" />
    </Container>
  );
}
