...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageBySlug('oem-odm', 'es');
  return {
...
export default async function SpanishOemOdmPage() {
  const page = await getPageBySlug('oem-odm', 'es');
    ...
    <Container className="py-12">
      ...
      {steps.length > 0 && (
        <section className="mt-16">
          <SectionHeading title={t('es', 'sectionOemProcessTitle')} align="left" />
      ...
      <section className="mt-16">
        <SectionHeading title={t('es', 'customQuoteTitle')} align="left" />
        <div className="mt-6 max-w-2xl">
          <InquiryForm sourcePage="/es/oem-odm" locale="es" />
        </div>
      </section>
    </Container>
  );
